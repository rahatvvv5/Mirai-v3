...
module.exports.run = async function ({ api, event, args }) {
    ...
    switch(subCommand) {
        case "on":
            saveVIPMode(true);
            return api.sendMessage("🔰𝐎𝐊 𝐎𝐧𝐥𝐲 𝐕𝐈𝐏 𝐮𝐬𝐞𝐫 𝐜𝐚𝐧 𝐮𝐬𝐞 𝐜𝐦𝐝𝐬", event.threadID);

        case "off":
            saveVIPMode(false);
            return api.sendMessage("🔰𝐃𝐨𝐧𝐞 𝐚𝐥𝐥 𝐮𝐬𝐞𝐫 𝐜𝐚𝐧 𝐮𝐬𝐞 𝐜𝐦𝐝𝐬", event.threadID);
        ...
        case "list":
            if (vipList.length === 0) return api.sendMessage("✳️𝐕𝐢𝐩 𝐥𝐢𝐬𝐭 𝐢𝐬 𝐞𝐦𝐩𝐭𝐲.", event.threadID);
